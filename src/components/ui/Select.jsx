...
const Select = React.forwardRef(
  (
    ...
    // React 18+ stable id (no Math.random during render)
    const reactId = useId();
    const selectId = useMemo(() => {
      const safe = String(reactId).replace(/:/g, "");
      return id || `select-${safe}`;
    }, [id, reactId]);
    ...
    const handleOptionSelect = (option) => {
      ...
      if (multiple) {
        const arr = Array.isArray(value) ? value : [];
        const updatedValue = arr.includes(option.value)
    ...
    // If options change while open, keep search working without UI change
    useEffect(() => {
      (() => {
      ...
      <div ref={wrapperRef} className={cn("relative", className)}>
        ...
        <div className="relative">
          ...
          <select
            ...
            value={
              multiple
                ? "" // keep it inert; your custom UI controls selection
                : value || ""
            }
            onChange={() => {}}
            className="sr-only border-border"
            tabIndex={-1}
            multiple={multiple}
            required={required}
            aria-hidden="true"
